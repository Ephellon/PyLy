...
def run_pipeline(
   ...
   Returns:
      {"status": "ok"|"skipped"|"dry_run"|"redownloaded", "lrc": "<path>", "reason": "..."}
   """
   ...
   from .lyrics_fetch import (
      fetch_lyrics_data,
      fetch_by_url,
      read_pyly_url_from_lrc,
      build_lrc_metadata_headers,
   ...
   # ------------------------------------------------------------------
   # RE-DOWNLOAD MODE: read [PyLy:<url>] from existing .lrc and refresh
   # ------------------------------------------------------------------
   if redownload and out_lrc.exists():
      embedded_url = read_pyly_url_from_lrc(out_lrc)
      if embedded_url:
         _log(f"REDOWNLOAD: found URL {embedded_url}")
         print(info(f"REDOWNLOAD: {embedded_url}"))
         re_fetched = fetch_by_url(embedded_url, log_fn=_log)
         if re_fetched and re_fetched.synced_lrc_text:
            if not overwrite:
               _log("REDOWNLOAD: skipped (file exists, --overwrite not set)")
               return {"status": "skipped", "lrc": str(out_lrc), "reason": "existing .lrc"}

            # Build metadata headers from re-fetched data
            headers = build_lrc_metadata_headers(
               fetched=re_fetched,
               audio_path=audio_path,
               include_standard_tags=lrc_header,
            ) if lrc_header else ["[re:PyLy]", "[by:PyLy]"]

            # Inject headers into the synced LRC text
            synced_lines = re_fetched.synced_lrc_text.splitlines()
            write_lrc(out_lrc, synced_lines, overwrite=True, headers=headers)
            _log(f"REDOWNLOAD: wrote {out_lrc.name}")
            return {"status": "redownloaded", "lrc": str(out_lrc)}
         else:
            _log("REDOWNLOAD: fetch failed or no synced LRC; continuing with normal pipeline")
            print(warn("REDOWNLOAD: no synced lyrics from URL; continuing with Whisper pipeline"))
      else:
         _log("REDOWNLOAD: no [PyLy:url] tag found in existing .lrc")
         print(info("REDOWNLOAD: no embedded URL found; continuing with normal pipeline"))

   # ------------------------------------------------------------------
   # Fetch online lyrics (for base alignment or primary/alternate modes)
   # ------------------------------------------------------------------
   fetched = None
   ...
   if mode == "primary" and fetched and fetched.synced_lrc_text:
      ...
      write_lrc(out_lrc, synced_lines, overwrite=overwrite, headers=headers)
   ...
   try:
      ...
      # ---- Build LRC headers (metadata) ----
      headers: list[str] = []
      if lrc_header:
         headers = build_lrc_metadata_headers(
            fetched=fetched,
            audio_path=audio_path,
            include_standard_tags=True,
         )
      ...
      if base_lines:
         ...
         # Update headers with base alignment stats
         if lrc_header:
            if gsim is not None:
      ...
      if mode == "alternate":
         if fetched and fetched.synced_lrc_text:
            if out_fetched_lrc.exists() and not overwrite:
               ...
               alt_headers = build_lrc_metadata_headers(
                  fetched=fetched,
                  audio_path=audio_path,
                  include_standard_tags=lrc_header,
               ) if lrc_header else []
               alt_lines = fetched.synced_lrc_text.splitlines()
               write_lrc(out_fetched_lrc, alt_lines, overwrite=overwrite, headers=alt_headers)
